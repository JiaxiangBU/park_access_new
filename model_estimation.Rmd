...
distances <- st_distance(park_centroids, tract_centroids, byid = TRUE) %>%
...
notweets_logsum <- function(betas, distances, parks, tracts, model) {
  ...
  estimation_data <- tracts@data %>% 
    mutate(logsum = calculate_park_logsums(
      distances,  parks$log_size,
      betas = betas))
      
  sdm <- lagsarlm(update(model, . ~ . + logsum), data = estimation_data, 
...
access_sdm_val <- optim(
  ...
  lower = c(-100, -100), # size coefficient must be positive 
  upper = c(0, 100), # distance coefficient must be negative 
  hessian=TRUE, distances = log(distances[1:10, 1:10]), parks = parks[1:10,], tracts = tracts[1:10, ], 
  model = obese_base_lm, method = "L-BFGS-B", control = list(trace = 6))
```
...
tweets_logsum <- function(betas, distances, parks, tracts, model) {
  ...
  estimation_data <- tracts@data %>% 
    mutate(logsum = calculate_park_logsums(
      distances,  parks$log_size,  parks$log_tweets,
      betas = betas))
...
tweets_sdm_val <- optim(
  ...
  upper = c(-1e-9, 100, 100), # distance coefficient must be negative
  hessian=TRUE, distances = distances, parks = parks, tracts = tracts, 
  model = obese_base_lm, method = "L-BFGS-B", control = list(trace = 6))
...
tracts@data <- tracts@data %>%
  mutate(
    access_ls = calculate_park_logsums(distances, parks$log_size, 
                                       betas = notweets_lm_val$par),
    tweets_ls = calculate_park_logsums(distances, parks$log_size, parks$log_tweets, 
                                       betas = tweets_lm_val$par),
    # demean
    access_ls = access_ls - mean(access_ls),
    tweets_ls = tweets_ls - mean(tweets_ls)
  )

access_sdm <- lagsarlm(update(obese_base_sdm, .~ . + access_ls),   
                       data = tracts@data,  listw = W, zero.policy = TRUE, 
                       type = "mixed")
tweets_sdm <- lagsarlm(update(obese_base_sdm, .~ . + tweets_ls),   
                       data = tracts@data,  listw = W, zero.policy = TRUE, 
                       type = "mixed")
```
...
pa_access_sdm <- lagsarlm(update(base_formula, PHYSACT ~ . + notweets_ls),  
          data = tracts@data,  listw = W, zero.policy = TRUE, type = "mixed")
...
list("Obesity" = obesity_impacts, "Physical activity" = physact_impacts) %>%
  ...
  gather("Variable", "Impact", -Outcome, -effect) %>%
  filter(!is.na(Impact))  %>%
...
data_frame(
  ...
  Access = c(notweets_lm_val$par, NA),
  `Access + Tweets` = tweets_lm_val$par
)
