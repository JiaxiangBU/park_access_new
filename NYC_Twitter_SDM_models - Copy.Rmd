...
for zero variables as $log(0)$ is undefined.
...
leaflet(tracts %>% spTransform(., CRS("+init=epsg:4326"))) %>%
  ...
  addPolygons(group = "Long distance, no tweets", weight = 1, color = ~pal1(park_ls1)) %>%
  addPolygons(group = "Long distance, tweets", weight = 1, color = ~pal2(park_ls2)) %>%
  addPolygons(group = "Short distance, no tweets", weight = 1, color = ~pal4(park_ls4)) %>%
  addPolygons(group = "Short distance, tweets", weight = 1, color = ~pal5(park_ls5)) %>%
  addLayersControl(baseGroups =  c("Short distance, no tweets", 
                                   "Short distance, tweets", 
                                   "Long distance, no tweets", 
...
parks_formula_1a <- update(base_formula_age, . ~ . + park_ls1 )
parks_formula_2a <- update(base_formula_age, . ~ . + park_ls2 )
parks_formula_3a <- update(base_formula_age, . ~ . + park_ls4 )
parks_formula_4a <- update(base_formula_age, . ~ . + park_ls5 )
