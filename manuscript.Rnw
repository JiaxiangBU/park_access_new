...
leaves us with \Sexpr{nrow(parks)} separate parks. 

<<tractsdata, results="asis", message=FALSE, warning=FALSE, cache=FALSE>>=
tracts_data_index <- data_frame(
  variable =  c(
    "obesity", "physact", "density", "fulltime", "college", "single", 
...
tract_summary <- tracts@data %>%
  dplyr::select(obesity:Pct65plus) %>%
...
bind_rows(tract_summary, park_summary) %>%
  ...
  kable_styling() %>%
  group_rows(paste("Tract Variables, N =", nrow(tracts@data)), 1, 13) %>%
  group_rows(paste("Park Variables, N =", nrow(parks)), 14, 15)
...
termnames <- c("log(density)" , "Full-time workers", "College-educated", 
               "Single Adults", "Share of children (0-17)", "Share of young adults (18-29)", 
               "Share of seniors (65+)", "Black population share", "Asian population share", 
               "Hispanic population share", "Other Minorities")
@
...
texreg(pa_models, digits = 4, table = FALSE, single.row = TRUE,
       ...
       custom.coef.names = c("Intercept", termnames, paste("lag", termnames), "$\\rho$",
                             "Access logsum", "lag Access logsum", 
                             "Access + Tweets logsum", "lag Access + Tweets logsum"))
...
physact_impacts %>%
  ...
  filter(term %in% c("access_ls", "tweets_ls"), submodel == "Total") %>%
  dplyr::select(-term, -submodel) %>%
  kable("latex", booktabs = T, escape = FALSE,
        col.names = c("Logsum Measure", "Estimate", "$p$-value")) %>%
  footnote("$^{***}p<0.001$, $^{**}p<0.01$, $^*p<0.05$", escape = F)
@
\end{table*}
...
\Sexpr{round(effect_estimate * 100 * 6, 3)}\%. In other words, if a tract with 
poor accessibility is expected to achieve the sample mean of 
\Sexpr{round(mean(tracts$physact), 1)}\%, the same tract with excellent accessibility
is expected to achieve a rate of 
\Sexpr{round(mean(tracts$physact) * (1 + effect_estimate  * 6), 2)}\%. 
For an aggregate statistic strongly influenced by other socioeconomic factors, 
this is not an insubstantial marginal effect.

<<obesity_models>>=
obese_tweets_sdm <- update(pa_tweets_sdm, log_obesity ~ . + log_physact)
@
...
alongside the controlling variables and the tweets-augmented logsum variable. 
Table \ref{tab:obese_impacts} presents the simulated impacts resulting from
this model.

\begin{table*}
  \caption{Simulated Impacts of Variables on Obesity}
  \label{tab:obese_impacts}
<<tab:obese_impacts, results="asis">>=
extract_impacts(obese_tweets_sdm) %>%
  group_by(effect) %>%
  transmute(
    term,
    output = paste(round(`estimate`, 5),
                   gtools::stars.pval(`p-val`)),
    order = row_number()
  ) %>%
  spread(effect, output) %>%
  arrange(order) %>%
  dplyr::select(-order) %>%
  mutate(
    term = c(termnames, "Access + Tweets logsum", "log(Physical Activity Rate)")
  ) %>%
  kable("latex", booktabs = T, escape = FALSE, digits = 3, 
        col.names = c("", "Direct", "Indirect", "Total")) %>%
  footnote("$^{***}p<0.001$, $^{**}p<0.01$, $^*p<0.05$", escape = F)
@
\end{table*}
...
We readily acknowledge limitations in this study. To begin, it would be beneficial 
to have estimated the logsum coefficients from a high-quality survey rather than
asserting them to match {\em a priori expectations of}
