...
analysis estimates an effect twice as large but marginally less significant.
...
activity rates. A traditional buffer analysis does not show significance.
...
this masks a key difference in the specification worth remembering.
...
coefficient, on the other hand, suggest going from the least-accessible ($-3.0$)
