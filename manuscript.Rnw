...
termnames <- c("log(density)" , "log(Income)", 
...
texreg(pa_models, digits = 4, table = FALSE, single.row = TRUE,
       ...
       custom.coef.names = c("Intercept", termnames, "$\\lambda$",
                             "Access logsum", 
                             "Access + Tweets logsum"))
...
<<obesity_models>>=
obese_access_sdm <- update(pa_access_sdm, obesity ~ .)
obese_physact_sdm <- update(obese_access_sdm, . ~ . + physact)
@
...
extract_impacts(obese_physact_sdm) %>%
  group_by(effect) %>%
  transmute(
    term,
