...
We also collected data on park geometry, limited to parks larger than 0.5 acres.

\begin{table*}
  \caption{Summary Statistics of Parks Data}
  \label{tab:parksdata}
<<parks_summary, results="asis", message=FALSE, warning=FALSE, eval = FALSE>>=
park_summary <- list(
...
summary_table(as_data_frame(parks), park_summary) %>%
  kable("latex", booktabs = T, escape = FALSE, col.names = c("")) %>%
  group_rows("Size: size of park in acres", 1, 4) %>%
  group_rows("Tweets: number of tweets emanating from park", 5, 8)
@
\end{table*}
