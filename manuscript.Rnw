...
a destination-choice model \citep{Daly1982}. The extension
...
\log(\mathit{Obesity})_i = f(\vect{X}_i, A_i, g(W, \mathit(Obesity), \vect{X}))
\end{equation}
<<base_formula>>=
...
@
where $\vect{T}_i$ is a vector of tract-level attributes, $A_i$ is a choice-based
accessibility measure for the tract, and $g(W, \mathit(Obesity), \vect{T})$
...
tracts@data <- tracts@data %>%
  mutate(
    ...
    tweets_ls = calculate_park_logsums(log(distances), parks$log_size, parks$log_tweets,
                                       betas = c(-0.42, 1.0)),
    distance_ls = (distance_ls - mean(distance_ls) )/ sd(distance_ls),
...
leaflet(tracts_sf) %>%
  ...
  addPolygons(group = "access", color = ~ pal(access_ls), label = ~as.character(round(access_ls,2))) 
@
