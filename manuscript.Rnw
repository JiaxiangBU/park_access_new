...
Applying a more traditional analysis considering 1/4 mile acess to parks yields
...
analysis estimates an similar albeit marginally less significant effect.
...
comparing overall model likelihoods between it and specification where tracts
sharing a boundary are considered neighbors (queen-adjacent).
...
The logsum allow for variation in access unavailable in the buffer-based method.
...
on the expected sign, with a few exceptions: in this case an increasing hispanic
...
accessibility and these metrics, previous literature has been mixed. Using New
York City as a case study, we presented a holistic and flexible measurement for
...
distance may be arbitrarily asserted by the researcher. Adopting choice-based
accessibilities of the kind used in this study will allow researchers to
encompass the full universe of park amenities in their accessibility analyses.
