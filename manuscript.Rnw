...
and geolocated data collected from the Twitter API. Controlling for the 
...
greenways as key elements of the built environment that can support physical 
activity \citep{Bedimo-Rung2005a}.
...
rate half a percentage point lower than the most accessible tracts, though 
this result is only suggestive of correlation from a statistical perspective.
...
introducing the number of tweets emanating from within a park during 
...
number of scholars the measure them.''(p. 227) This is at least partly because
...
It defies reason to assume person living 1.1 miles 
...
statistics, in the same way that the gravity-based trip distribution model model 
...
calibrated and multimodal logsums are readily available \citep{Geurs2010}.
...
lapply(c(tweets_models, pa_models, obesity_models), function(model){
  ...
  dplyr::select(income, term, estimate, std.error, p.value) %>%
