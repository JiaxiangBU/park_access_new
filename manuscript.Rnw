...
accessibility to parks that exist in many cities. Some cities with large amounts of total
...
proportion of greenspace may give all of its citizens better access to sufficient
greenspace to meet their needs or wants for physical activity. Considering access at the neighborhood level within a single city
may also eliminate some regional or cultural fixed effects affecting
...
unique greenspace. We elect to retain these independent features as they may
...
SDM model log-likehood \cite. The constraint required that the coefficient on
...
resulted in $\beta_d = -1.98$  and $\beta_s = 0.000$. Though the "most likely"
...
the observation that in this specification tracts bordering on the corners of
Central Park were measured with poor overall accessibility. we subsequently adjusted the $\beta_s$ value
so that the
